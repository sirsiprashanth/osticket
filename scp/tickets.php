...
// return $org = Organization::lookup($_GET['orgid']);
...
if (!$ticket) {
    ...
    // Fetch the organization based on orgid if available.
    if (isset($_GET['orgid'])) {
        $orgId = (int)$_GET['orgid']; // Assuming orgid is an integer.
        $org = Organization::lookup($orgId); // Fetch the organization based on orgid.

        // Check if the organization exists and has an SLA end date.
        if ($org && isset($org->sla_end_date)) {
            $slaEndDate = strtotime($org->sla_end_date); // Convert SLA end date to a timestamp.
            $currentDate = strtotime('now'); // Get the current date as a timestamp.

            // Check if the SLA end date has passed.
            if ($slaEndDate < $currentDate) {
                // Throw an error and stop the user from loading the page.
                $errors['err'] = __('Organization\'s SLA has expired. Contact support for assistance.');
                require_once(STAFFINC_DIR . 'header.inc.php');
                require_once(STAFFINC_DIR . 'ticket-expired-error.inc.php'); // Create a custom error page.
    ...
    if ($ticket && $ticket->getId()) {
        ...
        switch (strtolower($_POST['a'])):
            case 'reply':
                ...
                if (!$errors && ($response = $ticket->postReply(
                    $vars,
                    $errors,
                    $alert
                ))) {
                ...
                switch (strtolower($_POST['do'])):
                    ...
                    case 'changeuser':
                        if (!$role->hasPerm(Ticket::PERM_EDIT)) {
                            ...
                            $msg = sprintf(
                                __('Ticket ownership changed to %s'),
...
$ost->addExtraHeader('<script type="text/javascript" src="js/ticket.js?0daca43"></script>');
$ost->addExtraHeader('<script type="text/javascript" src="js/thread.js?0daca43"></script>');
$ost->addExtraHeader(
